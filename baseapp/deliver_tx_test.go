...
import (
	...
	store "github.com/PikeEcosystem/cosmos-sdk/store/types"
...
func TestBeginBlock_WithInitialHeight(t *testing.T) {
	...
	require.PanicsWithError(t, "invalid height: 4; expected: 3", func() {
		app.BeginBlock(tmabci.RequestBeginBlock{
...
func TestCheckTx(t *testing.T) {
	...
	header := tmproto.Header{Height: 1}
	app.BeginBlock(tmabci.RequestBeginBlock{Header: header, Hash: []byte("hash")})
	...
	// reset CheckTx state
	app.BeginRecheckTx(tmabci.RequestBeginRecheckTx{Header: header})
...
func setupBaseAppWithSnapshots(t *testing.T, blocks uint, blockTxs int, options ...func(*BaseApp)) (*BaseApp, func()) {
	...
	for height := int64(1); height <= int64(blocks); height++ {
		app.BeginBlock(tmabci.RequestBeginBlock{Header: tmproto.Header{Height: height}})
...
func TestSetLoader(t *testing.T) {
	...
	for name, tc := range cases {
		...
		t.Run(name, func(t *testing.T) {
			...
			// "execute" one block
			app.BeginBlock(tmabci.RequestBeginBlock{Header: tmproto.Header{Height: 2}})
			res := app.Commit()
