...
import (
	...
	qtypes "github.com/PikeEcosystem/cosmos-sdk/types/query"
...
func (s IntegrationTestSuite) TestQueryNodeInfo() {
	...
	restRes, err := rest.GetRequest(fmt.Sprintf("%s/lbm/base/tendermint/v1/node_info", val.APIAddress))
	s.Require().NoError(err)
...
func (s IntegrationTestSuite) TestQuerySyncing() {
	...
	restRes, err := rest.GetRequest(fmt.Sprintf("%s/lbm/base/tendermint/v1/syncing", val.APIAddress))
	s.Require().NoError(err)
...
func (s IntegrationTestSuite) TestQueryLatestBlock() {
	...
	restRes, err := rest.GetRequest(fmt.Sprintf("%s/lbm/base/tendermint/v1/blocks/latest", val.APIAddress))
	s.Require().NoError(err)
...
func (s IntegrationTestSuite) TestQueryBlockByHash() {
	...
	restRes, err := rest.GetRequest(fmt.Sprintf("%s/lbm/base/tendermint/v1/block/%s", val.APIAddress, base64.URLEncoding.EncodeToString(blkhash)))
	s.Require().NoError(err)
...
func (s IntegrationTestSuite) TestQueryBlockByHeight() {
	...
	restRes, err := rest.GetRequest(fmt.Sprintf("%s/lbm/base/tendermint/v1/blocks/%d", val.APIAddress, 1))
	s.Require().NoError(err)
...
func (s IntegrationTestSuite) TestQueryBlockResultsByHeight() {
	...
	restRes, err := rest.GetRequest(fmt.Sprintf("%s/lbm/base/tendermint/v1/blockresults/%d", val.APIAddress, 1))
	s.Require().NoError(err)
...
func (s IntegrationTestSuite) TestQueryLatestValidatorSet() {
	...
	_, err = rest.GetRequest(fmt.Sprintf("%s/lbm/base/tendermint/v1/validatorsets/latest", val.APIAddress))
	s.Require().NoError(err)

	// rest request with pagination
	restRes, err := rest.GetRequest(fmt.Sprintf("%s/lbm/base/tendermint/v1/validatorsets/latest?pagination.offset=%d&pagination.limit=%d", val.APIAddress, 0, 1))
	s.Require().NoError(err)
...
func (s IntegrationTestSuite) TestLatestValidatorSet_GRPCGateway() {
	...
	testCases := []struct {
		...
		{"no pagination", fmt.Sprintf("%s/lbm/base/tendermint/v1/validatorsets/latest", vals[0].APIAddress), false, ""},
		{"pagination invalid fields", fmt.Sprintf("%s/lbm/base/tendermint/v1/validatorsets/latest?pagination.offset=-1&pagination.limit=-2", vals[0].APIAddress), true, "strconv.ParseUint"},
		{"with pagination", fmt.Sprintf("%s/lbm/base/tendermint/v1/validatorsets/latest?pagination.offset=0&pagination.limit=2", vals[0].APIAddress), false, ""},
	}
	for _, tc := range testCases {
		tc := tc
		s.Run(tc.name, func() {
...
func (s IntegrationTestSuite) TestValidatorSetByHeight_GRPCGateway() {
	...
	testCases := []struct {
		...
		{"invalid height", fmt.Sprintf("%s/lbm/base/tendermint/v1/validatorsets/%d", vals[0].APIAddress, -1), true, "height must be greater than 0"},
		{"no pagination", fmt.Sprintf("%s/lbm/base/tendermint/v1/validatorsets/%d", vals[0].APIAddress, 1), false, ""},
		{"pagination invalid fields", fmt.Sprintf("%s/lbm/base/tendermint/v1/validatorsets/%d?pagination.offset=-1&pagination.limit=-2", vals[0].APIAddress, 1), true, "strconv.ParseUint"},
		{"with pagination", fmt.Sprintf("%s/lbm/base/tendermint/v1/validatorsets/%d?pagination.offset=0&pagination.limit=2", vals[0].APIAddress, 1), false, ""},
	}
	for _, tc := range testCases {
		tc := tc
		s.Run(tc.name, func() {
