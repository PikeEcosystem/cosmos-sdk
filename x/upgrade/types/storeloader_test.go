	...
	for name, tc := range cases {
		...
		t.Run(name, func(t *testing.T) {
			...
			for i := int64(2); i <= upgradeHeight-1; i++ {
				origapp.BeginBlock(tmabci.RequestBeginBlock{Header: tmproto.Header{Height: i}})
				res := origapp.Commit()
			...
			// "execute" one block
			app.BeginBlock(tmabci.RequestBeginBlock{Header: tmproto.Header{Height: upgradeHeight}})
			res := app.Commit()
			require.NotNil(t, res.Data)
