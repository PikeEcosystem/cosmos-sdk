...
TMVERSION := $(shell go list -m github.com/PikeEcosystem/tendermint | sed 's:.* ::')
...
HTTPS_GIT := https://github.com/PikeEcosystem/cosmos-sdk.git
...
ifeq ($(LEDGER_ENABLED),true)
  ifeq ($(OS),Windows_NT)
    ...
    ifeq ($(UNAME_S),OpenBSD)
      $(warning OpenBSD detected, disabling ledger support (https://github.com/PikeEcosystem/cosmos-sdk/issues/1988))
    else
...
ldflags = -X github.com/PikeEcosystem/cosmos-sdk/version.Name=sim \
		  ...
		  -X "github.com/PikeEcosystem/cosmos-sdk/version.BuildTags=$(build_tags_comma_sep)" \
			-X github.com/PikeEcosystem/tendermint/version.TMCoreSemVer=$(TMVERSION)
...
mocks: $(MOCKS_DIR)
	...
	$(mockgen_cmd) -package mocks -destination tests/mocks/tendermint_tendermint_libs_log_DB.go github.com/PikeEcosystem/tendermint/libs/log Logger
.PHONY: mocks
...
format:
	...
	find . -name '*.go' -type f -not -path "./vendor*" -not -path "*.git*" -not -path "./client/docs/statik/statik.go" -not -path "./tests/mocks/*" -not -name '*.pb.go' | xargs goimports -w -local github.com/PikeEcosystem/cosmos-sdk
.PHONY: format
...
## Issue link: https://github.com/PikeEcosystem/tendermint/issues/5021
