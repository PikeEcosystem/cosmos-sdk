...
import (
	...
	"github.com/PikeEcosystem/cosmos-sdk/x/staking/simulation"
...
func TestSimulateMsgCreateValidator(t *testing.T) {
	...
	// begin a new block
	app.BeginBlock(tmabci.RequestBeginBlock{Header: tmproto.Header{Height: app.LastBlockHeight() + 1, AppHash: app.LastCommitID().Hash}})
...
func TestSimulateMsgEditValidator(t *testing.T) {
	...
	// begin a new block
	app.BeginBlock(tmabci.RequestBeginBlock{Header: tmproto.Header{Height: app.LastBlockHeight() + 1, AppHash: app.LastCommitID().Hash, Time: blockTime}})
